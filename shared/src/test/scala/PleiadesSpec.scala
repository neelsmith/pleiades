...
class PleiadesSpec extends FlatSpec {
  ...
  "A Pleiades collection" should "be constructable" in pending
  it should "lookup an Option for records by ID" in pending
  it should "lookup a Vector of records by name" in pending
}


  /* {
    val places = Pleiadesces.parsePlacesCex(cex)
    places match {
